      ...
      <div id="addUserStepBillingAddress" class="card">
        ...
        <div class="card-body">
          ...
          <div class="px-sm-4 px-3">
            <JoinPersonal @nextStep="nextStep" :bankList="bankList" :joinType="joinType" :user="userInfo" :isAfterReg="true" v-if="joinType === 'P'" />
            <JoinCompany @nextStep="nextStep" :bankList="bankList" :joinType="joinType" :user="userInfo" :isAfterReg="true" v-else />
          </div>
        </div>
...
import { onMounted, onUnmounted, readonly, ref, PropType } from 'vue';
...
import { showAlert, showLogConsole, showModal, hideModal } from '@/utils/common-utils';
...
onMounted(() => {
  ...
  userInfo.value = props.member;
  showLogConsole(userInfo.value);
});
...
const nextStep = () => {
  showAlert('사업자정보 등록이 완료되었습니다.', 'success', () => {
    emits('getUserInfo');
  });
};
</script>
