<template>
  <PageNavigator :before_link="[]" :current="'1:1문의 관리'" />
        ...
        <table class="table table-align-middle border card-table table-vertical-border-striped table-bordered table-nowrap">
          ...
          <tbody>
            <tr class="text-center" v-for="(item, i) in qnaList.datas" :key="item.id">
              ...
              <td>{{ item.q_member.name }}</td>
              <td>{{ item.status === 'R' ? '답변대기중' : item.status === 'C' ? '답변완료' : '-' }}</td>
              <td>
                <div v-if="item.status === 'R'">
                  <button type="button" class="btn btn-sm btn-warning" @click.prevent="router.push({ path: `/setting/community/qna/detail`, state: { id: item.id } })">답변하기</button>
                </div>
                <div v-if="item.status === 'C'">
                  <button type="button" class="btn btn-sm btn-success" @click.prevent="router.push({ path: `/setting/community/qna/detail`, state: { id: item.id } })">상세보기</button>
                </div>
              </td>
            </tr>
...
const getQnaList = (init: boolean = true) => {
  ...
  let query = '';
  setSearchInfo(query);
  apis.community.get_qna_list(query, offset.value, limit.value).then(res => {
