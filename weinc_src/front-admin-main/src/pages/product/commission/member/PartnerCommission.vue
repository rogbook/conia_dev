<template>
  <PageNavigator :before_link="['회원 수수료 설정']" :current="'회원 수수료 상세 설정'" />
  <div class="card mb-4">
    <div class="card-header pb-1">
      <div class="form-control-borderless h2">회원 수수료 설정 - [{{ mMemberInfo.name }}{{ mMemberInfo.email ? ` (${mMemberInfo.email})` : '' }}]</div>
    </div>
    <div class="card-body">
      <div class="row col mb-2 align-items-center">
        <label class="col-md-2 col-form-label">하위 회원에게 받는 수수료율 설정 (0.01% ~ 100%)</label>
        <div class="col-md-2">
          <div class="input-group">
            <input type="text" class="form-control" inputmode="decimal" placeholder="기본 수수료율을 입력해주세요 (0.01% ~ 100%)" v-model="mDefaultCommission" @input="checkCommissionDefault($event)" />
            <span class="input-group-text">%</span>
          </div>
        </div>
        <div class="col-auto">
          <button type="button" class="btn btn-sm btn-outline-info" @click.prevent="modDefaultCommission" v-if="mDefaultCommission !== mDefaultCommissionOrigin">저장</button>
        </div>
      </div>
    </div>
...
const openNewCommission = () => {
  if (isExistChild.value) {
    //@ts-ignore
    selSubStoreList.selectedItem = selSubStoreList.items[0].store_code;
...
onMounted(() => {
  ...
  if (!memberId) {
    ...
    if (checkPermission('write:sub_commission') || getUserClassStr.value.includes('CM')) {
      getSubStores();
    } else {
      showAlert('하위 수수료 설정 권한이 없습니다.', 'error', () => {
        if (window.history.length > 1) {
          router.back();
        } else {
          router.replace('/');
