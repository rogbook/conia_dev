<template>
  <PageNavigator :before_link="['회원 수수료 설정']" :current="'중개 수수료 상세 설정'" />
  <div class="card mb-4">
    <div class="card-header py-3">
      <div class="row align-items-center justify-content-between">
        <div class="form-control-borderless h2 col">중개수수료 설정 - [대상자 : {{ mMemberInfo.name }}{{ mMemberInfo.email ? ` (${mMemberInfo.email})` : '' }}]</div>
        <div class="col text-end">
          <button type="button" class="btn btn-sm btn-warning" @click.prevent="showModal('addNewCommission')">중개 수수료 설정 추가</button>
        </div>
      </div>
    </div>
  </div>
  <span class="divider-center py-4">설정된 중개수수료 목록</span>
  ...
  <table class="table table-nowrap table-align-middle card-table">
    ...
    <tbody>
      ...
      <tr class="text-center" v-for="(commission, i) in mCommissionList.datas" :key="commission.id">
        <td>{{ commission.member1.name }} ({{ commission.member1.email }})</td>
        ...
        <td v-else>-</td>
        <td style="width: 25%">
          <div class="row align-items-center justify-content-center" v-if="commission.type === 'P'">
            <div class="col-auto">
              <div class="input-group">
                <input type="text" class="form-control text-end" inputmode="decimal" placeholder="수수료율을 입력해주세요 (0.01% ~ 100%)" v-model="commission.value" @input="checkCurrentCommission($event, i)" />
          ...
          <div class="row align-items-center justify-content-center" v-else-if="commission.type === 'F'">
            <div class="col-auto">
              <div class="input-group">
                <input type="text" class="form-control text-end" inputmode="decimal" placeholder="수수료 금액을 입력해주세요" v-model="commission.value" @input="checkCurrentCommission($event, i)" />
        ...
        <td>
          {{ commission.payment === 'D' ? '이익차감' : commission.payment === 'S' ? '분리지급' : '-' }}
        </td>
        <td>
      ...
      <tr>
        <td colspan="6" class="text-center" v-if="mCommissionList.total == 0">표시할 항목이 없습니다.</td>
      </tr>
    </tbody>
  </table>
  <!-- Pagination -->
  ...
  <!-- 추가 수수료 등록 Modal -->
  <Modal :id="'addNewCommission'" :title="'중개 수수료 설정 등록'">
    <template #body>
      <div class="card">
        <div class="card-body">
          <div class="row col mb-2 align-items-center">
            <label class="col-md-3 col-form-label">지급 대상 회원</label>
            <div class="col">
              <div class="input-group">
          ...
          <div class="row col align-items-center mb-2">
            ...
            <div class="col-auto">
              <div class="row align-items-center">
                <div class="col-auto">
                  <input type="radio" id="radio_kind_all" class="form-check-input" name="radio_status" value="all" v-model="newCommissionInfo.kind" />
                  <label class="form-check-label px-1" for="radio_kind_all">전체</label>
                </div>
                <div class="col-auto">
                  <input type="radio" id="radio_kind_store" class="form-check-input" name="radio_status" value="store" v-model="newCommissionInfo.kind" />
                  <label class="form-check-label px-1" for="radio_kind_store">상점</label>
                </div>
                <div class="col-auto">
                  <input type="radio" id="radio_kind_prod" class="form-check-input" name="radio_status" value="prod" v-model="newCommissionInfo.kind" />
                  <label class="form-check-label px-1" for="radio_kind_prod">상품</label>
                </div>
              </div>
            </div>
          </div>
          <div class="row col mb-2 align-items-center" v-if="newCommissionInfo.kind === 'store'">
            ...
            <div class="col">
              <div class="input-group" v-if="!mMemberInfo.classStr.includes('MC')">
              ...
              <div class="input-group" v-else>
                <input type="text" class="form-control" placeholder="대상 상점을 선택해주세요." :value="newCommissionInfo.store_title" disabled />
              </div>
            </div>
          </div>
          ...
          <div class="row col align-items-center mb-2">
            <label class="col-md-3 col-form-label">수수료 방식</label>
            <div class="col-auto">
              <div class="row align-items-center">
                <div class="col-auto">
                  <input type="radio" id="radio_type_p" class="form-check-input" name="radio_type" value="P" v-model="newCommissionInfo.type" @change="commTypeChange" />
                  <label class="form-check-label px-1" for="radio_type_p">퍼센트</label>
                </div>
                <div class="col-auto">
                  <input type="radio" id="radio_type_f" class="form-check-input" name="radio_type" value="F" v-model="newCommissionInfo.type" @change="commTypeChange" />
                  <label class="form-check-label px-1" for="radio_type_f">고정금액</label>
                </div>
              </div>
            </div>
          </div>
          <div class="row col mb-2 align-items-center" v-if="newCommissionInfo.type === 'P'">
            <label class="col-md-3 col-form-label">수수료율 설정 (0.01% ~ 100%)</label>
            <div class="col-auto">
              <div class="input-group">
                <input type="text" class="form-control text-end" inputmode="decimal" placeholder="기본 수수료율을 입력해주세요 (0.01% ~ 100%)" v-model="newCommissionInfo.value" @input="checkNewCommission($event)" />
                <span class="input-group-text">%</span>
              </div>
            </div>
          </div>
          <div class="row col mb-2 align-items-center" v-else-if="newCommissionInfo.type === 'F'">
            <label class="col-md-3 col-form-label">수수료 금액설정</label>
            <div class="col-auto">
              <div class="input-group">
                <input type="text" class="form-control text-end" inputmode="decimal" placeholder="기본 수수료율을 입력해주세요 (0.01% ~ 100%)" v-model="newCommissionInfo.value" @input="checkNewCommission($event)" />
                <span class="input-group-text">원</span>
          ...
          <div class="row col align-items-center mb-2">
            <label class="col-md-3 col-form-label">지급 유형</label>
            <div class="col-auto">
              <div class="row align-items-center">
                <div class="col-auto">
                  <input type="radio" id="radio_payment_d" class="form-check-input" name="radio_payment" value="D" v-model="newCommissionInfo.payment" />
                  <label class="form-check-label px-1" for="radio_payment_d">이익 차감</label>
                </div>
                <div class="col-auto">
                  <input type="radio" id="radio_payment_s" class="form-check-input" name="radio_payment" value="S" v-model="newCommissionInfo.payment" />
                  <label class="form-check-label px-1" for="radio_payment_s">분리 지급</label>
                </div>
              </div>
            </div>
          </div>
  ...
  <Modal :id="'searchProdModal'" :title="'수수료 설정 대상 상품 선택'" :xlarge="true" :second="true">
    <template #body>
      <AllProdListModal ref="prodModal" @selectProd="selectProd" :pa-id="mMemberId" :pa-name="`${mMemberInfo.name} (${mMemberInfo.email})`" v-if="mMemberInfo.classStr?.includes('PA')" />
      <StoreProdListModal ref="prodModal" @selectProd="selectProd" :store-code="mStoreInfo.code" :store-title="mStoreInfo.title" v-if="mMemberInfo.classStr?.includes('MC')" />
    </template>
    <template #footer>
          ...
          <div class="card-body">
            ...
            <div class="mb-6">
              <div class="row col">
                ...
                <div class="col">
                  <div class="input-group">
                    <input type="text" class="form-control" id="q" v-model="selDetailSearchMember.q" :placeholder="selDetailSearchMember.placeholder" @keypress.enter.prevent="reqUserList" />
                    <button type="button" class="btn btn-outline-dark col-md-2" @click.prevent="reqUserList(true)">검색</button>
                  </div>
                </div>
              </div>
            </div>
              ...
              <table class="table table-lg table-borderless table-thead-bordered table-nowrap table-align-middle card-table table-nowrap">
                ...
                <tbody>
                  <tr class="text-center" v-for="(user, i) in searchUserList.data" :key="user.id">
                    ...
                    <td>{{ user.mobile }}</td>
                    <td>
                      <button type="button" class="btn btn-sm btn-info" @click.prevent="setMemberInfo(user)" :disabled="user.classes.length === 0">{{ user.classes.length === 0 ? '선택불가' : '선택' }}</button>
                    </td>
                  </tr>
                  <tr>
            ...
            <div class="table-footer-area" v-if="searchUserList.total > 0">
              <div class="row" v-if="user_total_page > 1">
                <Pagination :currentPage="user_page_no" :totalPages="user_total_page" :pageChange="userPageChange" />
              </div>
            </div>
            <!-- End Pagination -->
          </div>
...
import StoreProdListModal from '@/pages/product/commission/modal/StoreProdListModal.vue';

const mMemberId = ref(0);
const mMemberInfo = ref({} as { name: string; email: string; classStr: string; storeCode: string; storeTitle: string });
...
const userPageChange = (page: number) => {
...
const commTypeChange = () => {
  console.log(newCommissionInfo.type);
  switch (newCommissionInfo.type) {
    case 'P':
...
const getCommissionList = (init: boolean = true) => {
  ...
  //TODO: 쿼리스트링 작업 예정
  let query = `&member_id=${mMemberId.value}&`;
...
const addNewCommission = () => {
  // 대상 체크
  if (newCommissionInfo.target === 0) {
    showAlert('대상 회원을 선택해주세요', 'warning');
    return;
  }
  ...
  // TODO: kind 설정 ?
  let data = { member_id: mMemberId.value, value: newCommissionInfo.value, type: newCommissionInfo.type, target: newCommissionInfo.target, payment: newCommissionInfo.payment, kind: 'prd' } as any;
  if (newCommissionInfo.kind === 'store') {
...
const modListCommission = (idx: number) => {
  ...
  if (mCommissionList.value.datas[idx].type === 'P') {
    ...
    if (!mCommissionList.value.datas[idx].value) {
      showAlert('수수료 금액을 0원으로 설정할 수 없습니다.', 'warning');
      return;
    }

    if (mCommissionList.value.datas[idx].value > 99999999) {
      showAlert('수수료 금액은 1억 미만으로 설정할 수 없습니다.', 'warning');
      return;
    }
  }
  ...
  if (newCommissionInfo.type === 'F') {
    if (e.target.value.startsWith('00') || /0\d$/.test(e.target.value)) {
      ...
      if (e.target.value > 99999999) {
        showAlert('최대 1억 미만까지 입력할 수 있습니다.', 'warning', () => {
          e.target.focus();
        });
      }
    }
...
const checkCurrentCommission = (e: any, idx: number) => {
  ...
  if (mCommissionList.value.datas[idx].type === 'F') {
    if (e.target.value.startsWith('00') || /0\d$/.test(e.target.value)) {
      e.target.value = parseFloat(e.target.value as string);
    } else {
      if (e.target.value > 99999999) {
...
const clearNewCommissionInfo = () => {
  newCommissionInfo.kind = 'all';
  newCommissionInfo.value = 0.01;
  newCommissionInfo.type = 'P';
  newCommissionInfo.store_code = '';
  newCommissionInfo.store_title = '';
  ...
  newCommissionInfo.payment = 'D';
};

const setModalListener = () => {
  //@ts-ignore
  document.getElementById('addNewCommission').addEventListener('show.bs.modal', function (event) {
    if (mMemberInfo.value.classStr.includes('MC')) {
      newCommissionInfo.store_code = mStoreInfo.code;
      newCommissionInfo.store_title = mStoreInfo.title;
    }
  });
...
onMounted(() => {
  ...
  const storeTitle = history.state.storeTitle;
  ...
  if (!memberId) {
    ...
    console.log(mMemberInfo.value);
    mStoreInfo.code = storeCode ? storeCode : '';
    mStoreInfo.title = storeTitle ? storeTitle : '';

    if (getUserClassStr.value.includes('CM')) {
      getCommissionList();
    } else {
      showAlert('수수료 설정 권한이 없습니다.', 'error', () => {
        if (window.history.length > 1) {
          router.back();
        } else {
          router.replace('/');
