<template>
  <PageNavigator :before_link="[]" :current="'자동발급 쿠폰관리'" />
  <div class="card">
    <div class="card-header">
      <div class="row align-items-center justify-content-between">
        <div class="col-auto">
          <div class="form-control-borderless h2 mb-0">자동발급 쿠폰관리</div>
        </div>
      </div>
    </div>
    <div class="card-body">
    ...
    <table class="table table-lg table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
      ...
      <tbody>
        <tr class="text-center" v-for="(item, i) in couponList.datas" :key="item.id">
          ...
          <td>
            <span v-if="item.percent">{{ item.percent }}%</span> <span v-else-if="item.amount">{{ item.amount.toLocaleString() }}원</span>
          </td>
          <td>{{ item.status === 'Y' ? '발행중' : item.status === 'N' ? '발행중지' : '-' }}</td>
          <td>
...
const onChangeDetailSearch = () => {
  switch (selDetailSearch.selectedItem) {
    case 'name':
      ...
      break;
    case 'auto_type':
      selDetailSearch.placeholder = '검색할 쿠폰의 쿠폰유형을 입력해주세요.';
      break;
  }
};
