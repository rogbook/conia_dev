<template>
  <PageNavigator :before_link="[]" :current="'PG 정산내역'" />
  <div class="card mb-4">
    <div class="card-header pb-1">
      <div class="form-control-borderless h2">PG 정산내역 - [{{ userInfo.name }}{{ userInfo.email ? ` (${userInfo.email})` : '' }}]</div>
    </div>
    <div class="card-body">
      <!-- 정산상태 Checkbox -->
    ...
    <table class="table table-nowrap table-align-middle card-table table-bordered">
      <thead>
        <tr class="thead-light">
          <th class="text-end" :colspan="getUserClassStr.includes('CM') ? 11 : 9">정산 금액 합계</th>
      ...
      <tbody>
        <tr class="text-center" v-for="s in mSettlementList.datas" :key="s.id">
          ...
          <td>
            <button type="button" class="btn btn-sm btn-outline-info" style="font-weight: bold" @click.prevent="router.push({ name: 'orderDetail', state: { orderId: s.account_raw.order_id } })">{{ s.account_raw.order_id }}</button>
          </td>
          <td>{{ s.account_raw.product.name }}</td>
          <td>{{ s.account_raw.store_code }}</td>
        ...
        <tr v-if="mSettlementList.total === 0">
          <td class="text-center" :colspan="getUserClassStr.includes('CM') ? 11 : 9">정산 내역이 없습니다.</td>
        </tr>
      </tbody>
    </table>
...
import { apiResponseCheck, dateTimeFormatConverter, getUserClassStr, showAlert, showLogConsole, showModal, hideModal, showConfirm, showConfirmInput } from '@/utils/common-utils';
import apis from '@/apis';
import { useRouter } from 'vue-router';
import type { Settlement, SettlementList } from 'SettlementInfoModule';
