<template>
  <PageNavigator :before_link="['정산 내역']" :current="'정산 내역 상세'" />
  <div class="card mb-4">
    <div class="card-header pb-1">
      <div class="form-control-borderless h2">정산 내역 - [{{ userInfo.name }} {{ userInfo.email ? `(${userInfo.email})` : '' }}, 업체명: {{ userInfo.company }}]</div>
    </div>
    <div class="card-body">
      <!-- 정산상태 Checkbox -->
      ...
      <div class="row align-items-center">
        <label for="idLabel" class="col-md-1 col-form-label form-label">정산내역</label>
        <div class="col form-control border-0">
          <div class="row form-control border-0">
            <div class="col-auto form-check form-check-inline">
              <input type="radio" id="radio_filter_all" class="form-check-input" name="search_minus" :value="false" v-model="showMinus" />
              <label class="form-check-label px-1" for="radio_filter_all">전체 보기</label>
            </div>
            <div class="col-auto form-check form-check-inline">
              <input type="radio" id="radio_filter_minus" class="form-check-input" name="search_minus" :value="true" v-model="showMinus" />
              <label class="form-check-label px-1" for="radio_filter_minus">마이너스 정산만 보기</label>
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="row mb-2 align-items-center justify-content-between">
    <div class="col-auto">
      ...
      <button type="button" class="btn btn-sm btn-outline-info ms-4" @click.prevent="reqExcel" v-if="mSettlementList.total > 0">정산내역 엑셀 요청하기</button>
    </div>
      ...
      <table class="table table-nowrap table-align-middle card-table table-bordered">
        <thead>
          <tr class="thead-light">
            <th class="text-end" :colspan="getUserClassStr.includes('CM') ? 14 : 11">정산 금액 합계</th>
        ...
        <thead class="thead-light">
          <tr class="text-center">
            ...
            <th v-if="getUserClassStr.includes('CM')">수수료율(%) 또는 고정금액</th>
            <th>과세여부</th>
            <th>정산금액</th>
          </tr>
        </thead>
        <tbody>
          <tr class="text-center" v-for="s in mSettlementList.datas" :key="s.id">
            ...
            <td>
              <button type="button" class="btn btn-sm btn-outline-info p-2" style="font-weight: bold; font-size: 0.7rem" @click.prevent="router.push({ name: 'orderDetail', state: { orderId: s.account_raw.order_id } })">{{ s.account_raw.order_id }}</button>
            </td>
            <td>{{ dateTimeFormatConverter(`${s.account_raw.order_id.slice(0, 4)}-${s.account_raw.order_id.slice(4, 6)}-${s.account_raw.order_id.slice(6, 8)}`).slice(0, 13) }}</td>
            <td>{{ dateTimeFormatConverter(s.account_raw.order_product.complete_date).slice(0, 13) }}</td>
            <td style="max-width: 600px; min-width: 300px" class="text-wrap">{{ s.account_raw.product.name }}</td>
            <td>{{ s.account_raw.store.title }} ({{ s.account_raw.store_code }})</td>
            <td>{{ s.account_raw.pg_type }}</td>
            <td class="text-end">{{ s.account_raw.amount.toLocaleString() }}원</td>
            <td class="text-end" v-if="getUserClassStr.includes('CM')">{{ s.target_amount.toLocaleString() }}원</td>
            <td v-if="getUserClassStr.includes('CM')">{{ s.commission_type === 'P' ? `비율 (${s.commission_value}%)` : '고정' }}</td>
            <td class="">{{ s.tax ? (s.tax === 'Y' ? '과세' : '비과세') : '-' }}</td>
            <td class="text-end">
              <a class="text-decoration-underline" href="" @click.prevent="showHistory(s.id, 'prd')" v-if="getUserClassStr.includes('CM')">{{ s.amount.toLocaleString() }}원</a>
              <span v-else>{{ s.amount.toLocaleString() }}원</span>
            </td>
          </tr>
          <tr v-if="mSettlementList.total === 0">
            <td class="text-center" :colspan="getUserClassStr.includes('CM') ? 15 : 12">정산 내역이 없습니다.</td>
          </tr>
        </tbody>
      </table>
      <div class="table-footer-area" v-if="mSettlementList.total > 0">
...
import { apiResponseCheck, dateTimeFormatConverter, getUserClassStr, showAlert, showConfirm, showConfirmInput, showModal, hideModal, showLogConsole } from '@/utils/common-utils';
import apis from '@/apis';
import { useRouter } from 'vue-router';
import type { AccountRaw, OrderProduct, Settlement, SettlementList } from 'SettlementInfoModule';
...
const getSettlementList = (init: boolean = true) => {
  ...
  if (showMinus.value === true) {
    query = query.concat(`&minus=y`);
  }

  apis.settlement.get_list(userInfo.id, query, offset.value, limit.value).then(res => {
    apiResponseCheck(res, () => {
      ...
      currentQueryString.value = query;
    });
  });
};
  ...
  if (status === 'J') {
    showConfirmInput(`선택한 항목을 [${convertStatus(status)}] 처리 하시겠습니까?`, '보류 사유를 입력해주세요.', (res: string) => {
      ...
      mSelSettlementList.value.map(item => {
        ids.push((item as Settlement).id);
      });
...
const showReject = (se: Settlement) => {
...
const modSettlement = (data: { ids: number[]; status?: string; date_range?: string; reject?: string }) => {
  showLogConsole(data);
  apis.settlement.mod_settlements(data).then(res => {
    apiResponseCheck(res, () => {
...
const reqExcel = () => {
  let query = currentQueryString.value;
  const mType = userInfo.classStr.includes('-') ? userInfo.classStr.split('-')[0] : userInfo.classStr;
  query = query.concat(`&member_id=${userInfo.id}&member_type=${mType}`);
  apis.settlement.req_settlement_excel(query).then(res => {
    apiResponseCheck(res, () => {
      showAlert('정산내역 엑셀 요청이 완료되었습니다.\n정산내역 엑셀요청게시판에서 확인해주세요.', 'success');
    });
  });
};
...
const downloadExcel = async () => {
  ...
  settlementList.unshift([]);
  if (userInfo.classStr.includes('MC')) {
    settlementList.unshift(['업체명', userInfo.company, '상점코드', userInfo.storeCode]);
  } else {
  ...
  // 엑셀 다운로드
  XLSX.writeFile(book, `${userInfo.name} 정산내역 - ${currentSearchPeriod.value}.xlsx`);
};
...
const makeSettlement = (): any[] => {
  const list = [] as any[];
  if (userInfo.classStr.includes('PA')) {
    // TODO : PA 데이터
    list.push(['구매확정일', '주문번호', '판매상점명(코드)', '상품명', '옵션', '수량', '공급가', '정산금액', '과세여부', '공급가액', '세액', '합계', '정산상태']);
    ...
    for (const s of mSettlementList.value.datas) {
      const data = [] as any[];
      data.push(s.account_raw.order_product?.complete_date); //구매확정일
      data.push(s.account_raw.order_id); //주문번호
      data.push(`${s.account_raw.store.title} (${s.account_raw.store_code})`); // 판매상점명 (코드)
      data.push(s.account_raw.product.name); //상품명
      data.push(optionStr(s.account_raw.order_product)); //옵션
      data.push(s.account_raw.order_product.ea); //수량
      data.push(s.account_raw.supply_price.toLocaleString()); //공급가
      data.push(s.amount.toLocaleString()); //정산급액
      data.push(s.tax === 'Y' ? 'O' : 'X'); // 과세여부
      data.push(s.tax === 'Y' ? Math.round(s.amount / 1.1).toLocaleString() : s.amount.toLocaleString()); //공급가액
      data.push(s.tax === 'Y' ? (s.amount - Math.round(s.amount / 1.1)).toLocaleString() : ''); //세액
      data.push(s.tax === 'Y' ? (Math.round(s.amount / 1.1) + (s.amount - Math.round(s.amount / 1.1))).toLocaleString() : s.amount.toLocaleString()); //합계
      data.push(convertStatus(s.status)); //정산상태
      tTaxAmount += s.tax === 'Y' ? Math.round(s.amount / 1.1) : s.amount;
      tTax += s.tax === 'Y' ? s.amount - Math.round(s.amount / 1.1) : 0;
      total += s.tax === 'Y' ? Math.round(s.amount / 1.1) + (s.amount - Math.round(s.amount / 1.1)) : s.amount;
      list.push(data);
    }
    list.push(['합계', '', '', '', '', '', '', '', '', tTaxAmount.toLocaleString(), tTax.toLocaleString(), total.toLocaleString()]);
  } else {
    // TODO: MC, 코니아
    list.push(['구매확정일', '주문번호', '판매상점명(코드)', '상품명', '옵션', '수량', '판매단가', '결제금액', '정산금액', '정산상태']);
    let tTaxAmount = 0;
    let tTax = 0;
    let total = 0;
    for (const s of mSettlementList.value.datas) {
      ...
      data.push(optionStr(s.account_raw.order_product)); //옵션
      data.push(s.account_raw.order_product.ea); //수량
      data.push(s.account_raw.order_product.product_option.selling_price.toLocaleString()); //판매단가
      data.push((s.account_raw.order_product.product_option.selling_price * s.account_raw.order_product.ea).toLocaleString()); // 결제금액
      data.push(s.amount.toLocaleString()); // 정산금액
      data.push(convertStatus(s.status)); //정산상태
      tTaxAmount += s.account_raw.order_product.product_option.selling_price * s.account_raw.order_product.ea;
      total += s.amount;
      list.push(data);
    }
    list.push(['합계', '', '', '', '', '', '', tTaxAmount.toLocaleString(), total.toLocaleString()]);
  }
  return list;
};
  ...
  if (getUserClassStr.value.includes('CM')) {
    apis.user.get_user(userInfo.id.toString(), 'member').then(res => {
      apiResponseCheck(res, () => {
        ...
        userInfo.storeCode = (res as User).store[0] ? (res as User).store[0].code : '';
        userInfo.storeTitle = (res as User).store[0] ? (res as User).store[0].title : '';
      });
    });
  } else {
