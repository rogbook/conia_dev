<template>
  <PageNavigator :before_link="[]" :current="'배송비 정산 내역'" />
  <div class="card mb-4">
    <div class="card-header pb-1">
      <div class="form-control-borderless h2">배송비 정산 내역 - 회원선택</div>
    </div>
    <div class="card-body">
      <!-- 회원타입 Checkbox -->
    ...
    <table class="table table-nowrap table-align-middle card-table">
      ...
      <tbody>
        ...
        <tr class="text-center" v-for="(user, i) in userList.data" :key="user.id">
          ...
          <td>{{ user.name }}</td>
          <td>{{ user.company?.name ? user.company?.name : '없음' }}</td>
          <td>{{ user.email }}</td>
          <td>{{ dateTimeFormatConverter(user.reg_date) }}</td>
          <td>
            <button type="button" class="btn btn-sm btn-info" @click.prevent="router.push({ name: 'settlementShipList', state: { memberId: user.id, name: user.name, email: user.email, s_date: searchDate.sDate, e_date: searchDate.eDate } })">내역 보기</button>
          </td>
        </tr>
        <tr>
...
const reqUserList = (reset: boolean = true) => {
  ...
  // 정산상태
  query = query.concat(query ? `&status=${checkedStatus.value}` : `&status=${checkedStatus.value}`);
  ...
  setSearchInfo(query);
  apis.settlement.get_ship_settlement_members(query, offset.value, limit.value).then(res => {
