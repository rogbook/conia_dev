<template>
  <PageNavigator :before_link="[]" :current="'코니아 배송비 정산내역'" />
  <div class="card mb-4">
    <div class="card-header pb-1">
      <div class="form-control-borderless h2">코니아 배송비 정산내역 - [{{ userInfo.name }}{{ userInfo.email ? ` (${userInfo.email})` : '' }}]</div>
    </div>
    <div class="card-body">
      <!-- 정산상태 Checkbox -->
  ...
  <div class="row mb-2 align-items-center justify-content-between">
    <div class="col-auto">
      ...
      <button type="button" class="btn btn-sm btn-outline-info ms-4" @click.prevent="downloadExcel" v-if="mSettlementList.total > 0 && getUserClassStr.includes('CM')">배송비 정산내역 엑셀 다운로드</button>
    </div>
    <div class="col">
