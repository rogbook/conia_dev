<template>
  <PageNavigator :before_link="[]" :current="'코니아 정산내역'" />
  <div class="card mb-4">
    <div class="card-header pb-1">
      <div class="form-control-borderless h2">정산내역 - [{{ userInfo.name }}{{ userInfo.email ? ` (${userInfo.email})` : '' }}]</div>
    </div>
    <div class="card-body">
      <!-- 정산상태 Checkbox -->
  ...
  <div class="row mb-2 align-items-center justify-content-between">
    <div class="col-auto">
      ...
      <button type="button" class="btn btn-sm btn-outline-info ms-4" @click.prevent="downloadExcel" v-if="mSettlementList.total > 0 && getUserClassStr.includes('CM')">정산내역 엑셀 다운로드</button>
    </div>
    <div class="col">
      ...
      <table class="table table-nowrap table-align-middle card-table table-bordered">
        <thead>
          <tr class="thead-light">
            ...
            <th class="text-end text-danger">{{ mSettlementList.total_sum?.toLocaleString() }}원</th>
          </tr>
        </thead>
...
import type { OrderProduct, Settlement, SettlementList } from 'SettlementInfoModule';
...
const downloadExcel = async () => {
  ...
  settlementList.unshift([]);
  settlementList.unshift(['업체명', '코니아']);
