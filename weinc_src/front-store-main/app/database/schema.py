from sqlalchemy import Column, DECIMAL, Date, DateTime, ForeignKey, String, Text, text, Index, Table, Time
...
class BaseMixin:
    ...
    def update_optional(self, session: Session, auto_commit=False, **kwargs):
        change_data = []
        for col in self.all_columns():
            col_name = col.name
            if col_name in kwargs:
                before = getattr(self, col_name)
                if kwargs.get(col_name) and before != kwargs.get(col_name):
                    change_data.append({
                        col_name: {
                            "before": before,
...
class Brand(Base, BaseMixin):
    ...
    description = Column(String(255), comment='브랜드 설명')
...
class Member(Base, BaseMixin):
    ...
    shop_yn = Column(String(2), comment='매장 여부')
...
class NoticeInfoTemplate(Base, BaseMixin):
    ...
    id = Column(INTEGER(10), primary_key=True)
    item = Column(String(45), nullable=False, comment='품목')
...
class SettingValue(Base, BaseMixin):
    ...
    type = Column(String(32), comment='타입')
    name = Column(String(64), comment='이름')
    value = Column(String(255), comment='값')
...
class ShippingInfo(Base, BaseMixin):
    ...
    costs = relationship('ShippingCost', back_populates='info')


class Calculate(Base, BaseMixin):
    __tablename__ = 'calculate'
    __table_args__ = {'comment': '정산'}
...
class Coupon(Base, BaseMixin):
    ...
    member = relationship('Member', primaryjoin='Coupon.issuer == Member.id')
    customer = relationship('Customer')
    product = relationship('Product')


class CouponPhoto(Base, BaseMixin):
    __tablename__ = 'coupon_photo'
...
class FavoriteProductPropertyDetail(Base, BaseMixin):
    ...
    favorite_product_property = relationship('FavoriteProductProperty')
...
class MemberCompany(Base, BaseMixin):
    ...
    corp_number = Column(String(45), nullable=False, comment='법인번호(주민번호)')
...
class Product(Base, BaseMixin):
    ...
    name = Column(String(45), nullable=False, comment='상품명')
    type = Column(String(16), comment='타입')
    status = Column(String(16), nullable=False, server_default=text("'Y'"), comment='상태')
    ...
    min_purchase_limit = Column(INTEGER(11), comment='최소 구매수량')
    max_purchase_limit = Column(INTEGER(11), comment='최대 구매수량')
    ...
    coupon_yn = Column(String(1), server_default=text("'Y'"), comment='쿠폰 사용 여부')
    ...
    subtitle = Column(String(64), comment='부제목')
...
class ShippingCost(Base, BaseMixin):
    ...
    info = relationship('ShippingInfo', back_populates='costs')
...
class Store(Base, BaseMixin):
    ...
    themes = relationship('StoreTheme', back_populates="store", lazy="joined")
...
class Customer(Base, BaseMixin):
    ...
    grade = Column(String(45), comment='등급')
    refresh_token = Column(String(512))
    legacy_id = Column(String(45))
    legacy_pw = Column(String(512))
    legacy_shop = Column(String(45))
...
class FaqCategory(Base, BaseMixin):
    ...
    sort = Column(INTEGER(11), nullable=False, server_default=text("99"))
...
class ProductOption(Base, BaseMixin):
    ...
    inventory = relationship('Inventory', back_populates="options", uselist=False)
...
class ShippingArea(Base, BaseMixin):
    ...
    shipping_cost_id = Column(ForeignKey('shipping_cost.id'), nullable=False, index=True)
...
class StoreTheme(Base, BaseMixin):
    ...
    store = relationship('Store', primaryjoin='StoreTheme.store_code == Store.code', back_populates="themes")
...
class WishProduct(Base, BaseMixin):
    ...
    product_id = Column(ForeignKey('product.id', ondelete='CASCADE'), primary_key=True, nullable=False, index=True)
    reg_date = Column(DateTime, server_default=text("current_timestamp()"))
    store_code = Column(ForeignKey('store.code', ondelete='CASCADE'), nullable=False, index=True)
...
class Inventory(Base, BaseMixin):
    ...
    options = relationship('ProductOption', back_populates="inventory")
...
class OrderCoupon(Base, BaseMixin):
    ...
    order_product_id = Column(ForeignKey('order_product.id'), index=True)
...
class OrderProduct(Base, BaseMixin):
    ...
    product_id = Column(ForeignKey('product.id'), nullable=False, index=True)
    settlement_date = Column(DateTime, comment='정산처리일')
...
class OrderSheetProduct(Base, BaseMixin):
    ...
    id = Column(String(36), primary_key=True)
    order_sheet_id = Column(ForeignKey('order_sheet.id', ondelete='CASCADE'), nullable=False, index=True)
    product_option_id = Column(ForeignKey('product_option.id'), nullable=False, index=True)
    amount = Column(DECIMAL(10, 2), nullable=False, comment='금액')
    ea = Column(INTEGER(11), nullable=False, comment='수량')
    coupon_id = Column(ForeignKey('coupon.id'), index=True)
...
class ShippingAreaDetail(Base, BaseMixin):
    ...
    shipping_area_id = Column(ForeignKey('shipping_area.id'), nullable=False, index=True)
...
class PaycoAuthCode(Base, BaseMixin):
    __tablename__ = 'payco_auth_code'
    __table_args__ = {'comment': '페이코 임직원 구분 코드'}

    id = Column(INTEGER(10), primary_key=True)
    store_code = Column(ForeignKey('store.code'), nullable=False, index=True)
    code = Column(String(45), nullable=False)
    memo = Column(String(45))
