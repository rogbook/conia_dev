...
class CertFailEx(APIException):
    def __init__(self, category: str, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_400,
            msg=f"인증에 실패하였습니다.",
            detail="Cert failed.",
...
class UnableStoreEx(APIException):
    def __init__(self, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_401,
            msg=f"이용할 수 없는 상점입니다.",
            detail="Unable Store.",
...
class NotMatchSnsEx(APIException):
    def __init__(self, ex: Exception = None, store_code=None):
        super().__init__(
            status_code=StatusCode.HTTP_401,
...
class NotCertEmailEx(APIException):
    def __init__(self, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_401,
            msg=f"이메일 인증이 완료되지 않았습니다.",
            detail="Not cert email",
            code=f"AUTH{'13'.zfill(4)}",
            ex=ex,
        )


class DisableUserEx(APIException):
    def __init__(self, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_401,
            msg=f"이용 정지 상태 계정입니다.",
            detail="Not cert email",
            code=f"AUTH{'14'.zfill(4)}",
            ex=ex,
        )


class LegacyUserEx(APIException):
    def __init__(self, ex: Exception = None, data=None):
        super().__init__(
            status_code=StatusCode.HTTP_403,
            msg=f"구버전 사용자입니다.",
            detail=data,
            code=f"{StatusCode.HTTP_403}{'13'.zfill(4)}",
            ex=ex,
        )


class LegacyPWEx(APIException):
    def __init__(self, ex: Exception = None, data=None):
        super().__init__(
            status_code=StatusCode.HTTP_403,
            msg=f"패스워드를 확인해주세요.",
            detail=data,
            code=f"{StatusCode.HTTP_403}{'14'.zfill(4)}",
            ex=ex,
        )
...
class BadRequestEx(APIException):
    def __init__(self, reason: str = "", ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_400,
            msg=f"{reason}",
            detail="Bad Request.",
...
class ViewNotFoundEx(APIException):
    def __init__(self, page: str = None, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_404,
            msg=f"해당 페이지를 찾을 수 없습니다.",
            detail=f"Not Found Page",
            code=f"404",
            ex=ex,
        )


class WaitingOpenEx(APIException):
    def __init__(self, page: str = None, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_400,
            msg=f"해당 페이지를 찾을 수 없습니다.",
