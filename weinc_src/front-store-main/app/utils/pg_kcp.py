...
def payment(data: ReqData):
    c = conf()
    api_url = f"{c.PG_KCP_HOST}/gw/enc/v1/payment"
...
# 결제 취소 API
def kcp_cancel(data: CancelData):
    c = conf()
...
def make_sign_data(orgData):
    ...
    c = conf()
    cwd = os.getcwd()
    file_path = os.path.join(cwd, 'certificate/', c.PG_KCP_CERT_FILE)

    if not os.path.exists(file_path):
        file_path = os.path.join(cwd, 'app/certificate/', c.PG_KCP_CERT_FILE)
